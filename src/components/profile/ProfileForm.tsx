...
const ProfileForm = () => {
  ...
  useEffect(() => {
    ...
    if (profile) {
      setName(profile.username || user?.email || "");
      setIncome(Number(profile.monthly_income) || 3000);
  ...
  const handleFinancialSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      // Placeholder for savings_goals table integration
      const { error } = await supabase.from("savings_goals").insert({
        user_id: user?.id,
        name: savingsGoalName,
        target_amount: savingsGoal,
        deadline: savingsDate,
        current_amount: 0,
      });
      ...
      toast({
        title: "Error",
        description: error.message || "Failed to save financial goal.",
        variant: "destructive",
      });
    }
  };
  ...
  const handlePreferencesSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    // Preferences are local-only for now, as no schema table exists
    toast({
            ...
            <div className="flex flex-col sm:flex-row items-center gap-6">
              ...
              <div className="flex flex-col items-center sm:items-start gap-2">
                ...
                <p className="text-xs text-muted-foreground">
                  PNG, JPG or GIF files up to 2MB (disabled for now)
                </p>
              </div>
            </div>
            ...
            <form onSubmit={handlePreferencesSubmit} className="space-y-6">
              <div className="space-y-4">
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      <Label htmlFor="daily-challenges" className="text-base">
                        Daily Challenges
                      </Label>
                      <p className="text-sm text-muted-foreground">
